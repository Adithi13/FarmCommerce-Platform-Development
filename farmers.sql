...
-CREATE TABLE `addtocart` (
  ...
  `bid` int(10) NOT NULL,
  `pid` int(10S) NOT NULL,
  `quantity` int(20) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_bid` (`bid`),
  KEY `fk_pid` (`pid`),
  CONSTRAINT `fk_bid` FOREIGN KEY (`bid`) REFERENCES `user` (`id`),
  CONSTRAINT `fk_pid` FOREIGN KEY (`pid`) REFERENCES `addagroproducts` (`pid`)
  )ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
-- Indexes for dumped tables
- INSERT INTO `addtocart`(5,2,1,3)

--
-- DELIMITER //
-- CREATE PROCEDURE InsertTransaction(IN bid INT, IN pid INT, IN name VARCHAR(100), IN city VARCHAR(100), IN mobile VARCHAR(20), IN email VARCHAR(100), IN pincode VARCHAR(10), IN addr VARCHAR(300))
-- BEGIN
--     INSERT INTO Transaction (bid, pid, name, city, mobile, email, pincode, addr)
